...
blockSize = 300;  % tune this: big enough for parfor efficiency, small enough for frequent saves
fprintf('Starting power calculation for %d points...\n', N);
...
for block = 1:blockSize:N
    ...
    parfor j = 1:chunkSize          % or regular for
        tic
        rx_j = rxChunk(j);
        raysForRx = raysAllTxRx(:, j);      % {numTx × 1} for this receiver
        hasRays = ~cellfun(@isempty, raysForRx);
        ...
        if isscalar(allocatedTxPower)
            allocPowerValid = allocatedTxPower;  % same power for all BSs
        else
            allocPowerValid = allocatedTxPower(hasRays);
        end

        exp = calcPower(txValid, rx_j, raysValid, fc, allocPowerValid);
